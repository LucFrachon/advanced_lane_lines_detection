...
def get_warp_matrix(img_size, source_coords):
    ...
    dest_coords = np.float32([[source_coords[0, 0], img_size[1] - 1],
                              [source_coords[0, 0], 0],
                              [source_coords[3, 0], 0],
                              [source_coords[3, 0], img_size[1] - 1]])
...
def global_binary_map(image, M_cam, dist_coef, M_warp, dest_vertices):
    ...
    warped_l = warped_hls[:,:,1]  # Isolate S channel for gradient calculations

    # Make binary maps from x gradient:
    x_binary, x_sobel  = make_grad_map(warped_l, orient = 'x', 
    ...
    return combined_map, img_undist, warped, warped_hls, warped_l, x_binary, \
        yellows, lightness, saturation
...
def window_search(binary_map):
    ...
    # Extract left and right line pixel positions (in image-space coordinates):
    ...
    # Calculate polynomial's outcome values for each pixel along the y axis.
    # These predictions are already converted to real-world coordinates so we
    # convert them back to pixels to be able to draw the lines:
    left_fitx = line_left.predict_avg_poly(ploty) / pix_to_m[0]
    right_fitx = line_right.predict_avg_poly(ploty) / pix_to_m[0]

    # Set pixels belonging to left and right line in red and blue respectively:
    ...
    position = ((pix_to_m[0] * img_size[0] - line_right.line_base_pos - \
        line_left.line_base_pos) / 2)
...
def search_around_line(binary_map):
    ...
    # Identify non-zero pixels that are within the search corridors. These need to
    # be converted to real-world coordinates because fit coefficients were 
    # calculated so:

    nonzerox_m = nonzerox * pix_to_m[0]
    nonzeroy_m = nonzeroy * pix_to_m[1]
    margin_m = margin * pix_to_m[0]

    left_lane_inds = \
        (nonzerox_m > \
        (left_fit[0] * nonzeroy_m ** 2 + left_fit[1] * nonzeroy_m + \
        left_fit[2] - margin_m)) & \
        (nonzerox_m < \
        (left_fit[0] * nonzeroy_m ** 2 + left_fit[1] * nonzeroy_m + \
        left_fit[2] + margin_m))
    ...
    right_lane_inds = \
        (nonzerox_m > \
        (right_fit[0] * nonzeroy_m ** 2 + right_fit[1] * nonzeroy_m + \
        right_fit[2] - margin_m)) & \
        (nonzerox_m < \
        (right_fit[0] * nonzeroy_m ** 2 + right_fit[1] * nonzeroy_m + \
        right_fit[2] + margin_m))
...
def lambda_wrapper(img, M_cam, dist_coef, M_warp, dest_vertices, 
    ...
    # As long as we found a valid line less than n_search frames ago (and we 
    # aready found at least one valid line at some point), search 
    # around it:
...
if __name__ == '__main__':
    ...
    line_left = Line([0, 255, 0], img_size, pix_to_m,
        n_fits, n_search, pos_tol = .5, rad_tol = 0.5)
    line_right = Line([255, 0, 0], img_size, pix_to_m,
        n_fits, n_search, pos_tol = .5, rad_tol = 0.5)
